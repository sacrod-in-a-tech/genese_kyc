...
let UsersService = class UsersService {
    ...
    async create(dto, actor) {
        ...
        const { hash, salt } = await (0, password_util_1.hashPassword)(dto.password);
    ...
    async changePassword(id, dto, actor) {
        const user = await this.findOne(id);
        const isValid = await (0, password_util_1.verifyPassword)(dto.currentPassword, user.salt, user.passwordHash);
        if (!isValid) {
            throw new common_1.UnauthorizedException('Current password is incorrect');
        }
        const { hash, salt } = await (0, password_util_1.hashPassword)(dto.newPassword);
